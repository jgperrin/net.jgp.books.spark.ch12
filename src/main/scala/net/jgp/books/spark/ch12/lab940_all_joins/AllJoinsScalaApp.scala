...
object AllJoinsScalaApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dfLeft = spark.createDataFrame(rows, schema)
    dfLeft.show()

    rows = new ArrayList[Row]
    rows.add(RowFactory.create(int2Integer(3), "Value 3"))
    rows.add(RowFactory.create(int2Integer(4), "Value 4"))
    rows.add(RowFactory.create(int2Integer(4), "Value 4_1"))
    rows.add(RowFactory.create(int2Integer(5), "Value 5"))
    rows.add(RowFactory.create(int2Integer(6), "Value 6"))

    val dfRight = spark.createDataFrame(rows, schema)
    dfRight.show()
    ...
    for (joinType <- joinTypes) {
      ...
      val df = dfLeft.join(dfRight, dfLeft.col("id") === dfRight.col("id"), joinType)
