 ...
 * @author rambabu.posa
 */
object HigherEdInstitutionPerCountyUsingAliasScalaApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    institPerCountyDf
      .filter(higherEdDf.col("zip") === 27517)
      .show(20, false)
    ...
    institPerCountyDf = institPerCountyDf
      .join(censusDf, institPerCountyDf.col("county") === censusDf.col("countyId"), "left")

    println("Higher education institutions and county id with census")
    institPerCountyDf
      .filter(higherEdDf.col("zip") === 27517)
      .show(20, false)

    institPerCountyDf
      .filter(higherEdDf.col("zip") === 2138)
      .show(20, false)
    ...
    println(s"The combined list has ${institPerCountyDf.count} elements.")
